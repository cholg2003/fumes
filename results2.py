...
class ExamResultsSystem:
    ...
    def update_excel(self):
        ...
        for idx, row in enumerate(rows, start=2):
            ws.cell(row=idx, column=1).value = row[0]  # Name
            ws.cell(row=idx, column=2).value = row[1]  # English
            ws.cell(row=idx, column=3).value = row[2]  # Mathematics
            ws.cell(row=idx, column=4).value = row[3]  # Science
            ws.cell(row=idx, column=5).value = row[4]  # C.R.E
            ws.cell(row=idx, column=6).value = row[5]  # S.S.T
